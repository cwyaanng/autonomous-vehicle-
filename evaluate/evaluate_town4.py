...
SIMULATION = f"TEST_일반화실험_Town03_to_Town04_{NOW}"

def make_env(batch_size: int = 256):

    client, world, carla_map = connect_to_carla(town="Town04")
    points = (300, -100)
...
def make_vec_env(batch_size: int = 256):
...
def load_policy_from_zip(model_path: str, env: gym.Env, device: str = "cpu") -> SAC:
    ...
    custom 클래스(SACOfflineOnline) 구조 mismatch 때문에 에러가 난다.
    그래서 여기서는 zip 안에서 'policy' 부분(state_dict)만 꺼내서
    새 SAC 모델의 policy에 얹어주는 방식으로 사용한다.
    """
...
def evaluate(
    ...
    # VecEnv + 모델 로드
    env = make_vec_env(batch_size=batch_size)
    ...
    print("\n========== Evaluation Summary ==========")
    print(f"#Episodes          : {n}")
    print(f"Return (mean ± std): {mean_return:.2f} ± {std_return:.2f}")
    print(f"RouteComp (mean±sd): {mean_rc:.3f} ± {std_rc:.3f}")
...
if __name__ == "__main__":
    ...
    MODEL_PATH = "logs/일반화실험_Town03_to_Town01/20251210_115936/trained_1M_1M_1차.zip"
