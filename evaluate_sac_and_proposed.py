...
def make_env(category , town, points, batch_size: int = 256):
...
def make_vec_env(category ,town, points, batch_size: int = 256 ):
    return DummyVecEnv([lambda: make_env(category , town, points, batch_size)])
...
def evaluate(
    ...
    # VecEnv + 모델 로드
    env = make_vec_env( category ,town, points, batch_size=batch_size)
